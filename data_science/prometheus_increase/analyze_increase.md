...
When I first read that, I found it lacking in detail. But I needed a mathematical definition of `increase`, so I built one myself.

**Disclaimer:** I will not explain everything about the function, since some details are beyond the scope of this article. If you need an authoritative reference, [you can check the golang source code](https://github.com/prometheus/prometheus/blob/9c23509790a38e4f5ec38b0c60c91d2a4fb45bd0/promql/functions.go#L469-L472), which I found pretty easy to read (you need to know some high-school-level math, though). Some of my explanations are based on that source code.
...
Given `range := (rangeStart, rangeEnd)`, the first selected sample in `v[range]` will be `(t1, v1)`, where `t1 >= rangeStart`. The range doesn't need to match the sampled timestamps exactly, so it is possible to have `t1 > rangeStart`.
...
[If we look at the documentation of `rate`](https://prometheus.io/docs/prometheus/latest/querying/functions/#rate), it is very similar to `increase`, so I will only paste here the interesting bit:
...
Why net increase? Because it doesn't matter what happened in between. This is not my opinion, but a mathematical statement. If you add the consecutive differences, most of the elements will cancel out:
```
(v2 - v1) + (v3 - v2) + (v4 - v3) + ... + (vn - v(n-1)) = vn - v1
```
...
However, this is not the precise formula, as it doesn't take into consideration the extrapolation. In the next section, I'll combine everything into the full formula, which will actually look simpler than the current one.
...
What the documentation doesn't mention is that the rate is not actually multiplied by the range window, but by the extrapolated time interval, `(t1', tn')`. If we apply the formula for `increase`, we get:
```
increase(v[range]) = rate(v[range]) * (tn' - t1') = vn' - v1'
```

However, we will use an equivalent formula, based on the fact that the extrapolation is linear:
```
factor = (tn' - t1') / (tn - t1)
increase(v[range]) = (vn - v1) * factor
```

This is useful because it preserves the original values. And we can avoid some complexity by assuming that `factor = 1`, which is generally not too far from the truth. I personally haven't reached the situation where I needed to think about the value of `factor`.
...
Before diving in, just by looking at the cheat sheet, you can observe some features:
- Whenever the counter becomes constant, the `increase` has a pull-back to 0
- The same slopes in the counter can be seen in `increase`
- If a counter has a constant slope for a long enough time, then `increase` becomes constant
- If the counter updates more **frequently**, `increase` has a higher value

In the following sections, I'm going to explain these features.

## They're all just differences

This is how a swept `increase` works:
- Fix a time interval, D
- Treat your counter plot as an X-Y coordinate plot
- Start with two points on the plot whose x values are D seconds apart
- Record the difference in the y values
- Shift both points by an increment
- Repeat

You can imagine those two points at fixed distance moving continuously from one end of the plot to the other.

If the sample rate is constant and high, then you'll have a nice property: the `factor` used in the formula will roughly be the same, because the `range` is shifted in both ends, keeping `rangeEnd - rangeStart` constant. Then, looking at the extrapolated timestamps: `t1'` , `tn'`, the difference `tn' - t1'` will be a constant factor of `rangeEnd - rangeStart`.

If the `factor` is roughly the same, then the `increase` plot is essentially a scaled version of all those recorded differences. This means that you can compare the values at different times: if the increase at t=2h is 3 times higher than at t=1h, then the counter was growing three times as fast.

## Blips

Let's look at a simple pattern: an instant ascent followed by a plateau.
...
I'm claiming that the `increase` plot for this counter looks like a "blip" - something that increases rapidly and decreases rapidly after a short period of time.

I will try to convince you why that is, by looking at a continuous approximation of the counter above.
...
The derivative looks like a blip. If I increase the scaling factor, the slope in arctan will become more abrupt, and the blip will become thinner and longer. Here you can play with this on Desmos: https://www.desmos.com/calculator/brieb8qiay.
...
Bonus: there's also a mathematical way to compute differences in a similar way `increase` does. You do `f(x) - f(x - b)`, where `b` is the length of the range. Here you can play with it on Desmos: https://www.desmos.com/calculator/0qa7iqvucy.

## The hidden plateau

So, instant changes create a blip: instant increase, followed by a plateau whose length depends on the range, then instant decrease. Gradual changes also create a sort of a blip.
...
Well, I'd say it still looks like a blip: an increase followed by a plateau, followed by a decrease. But the increase is not instant. Actually the slope of the increase is almost the same as the slope of the ascension. It turns out that this is not a coincidence. The math can confirm this.
...
On the other hand, the more you increase the range, the longer the ascension gets. It looks like the `increase` plot is trying to look closer like the counter plot.
...
And now, it actually increases as we increase the range. Our previous intuition applies again, with one exception: the length of the plateau is not equal to the range.

What actually was happening before was that the `increase` plot was trying to catch up to the counter plot. But, because the endpoints of the range were too close, the plot was starting to forget its past too fast, and it never reached the real plateau.
...
The second and third slopes in `increase` are smaller by 1, compared to the ones in the counter, and this is not a coincidence. Then, we have a fake plateau, and the same slopes, but negative, and in a different order.

The example in this section really depends on the chosen range. If you apply the formula and sweep the ends of the range across the counter, you'll see why this happens.

Why do the slopes of `increase` have these particular values? Here's where it gets interesting.
...
This is how `increase` looks with other ranges, without going deep into them. Hopefully you already got used to the fact that, the bigger the range is, the bigger `increase` will look. It's like a baloon that gets inflated.
...
So, as the blips get wider they start stacking on each other. This shows us an underlying property of `increase`: the function is greater when the range is greater.

Now, back to the previous counter. This is what happens when we increase the range from 10s to 40s:
<div style="display:flex; justify-content:flex-start;">
